...
fn parse_part(p: &str) -> Part {
    ...
    let (x, m, a , s) = p.split(|x:char| !x.is_ascii_digit()).filter_map(|x| x.parse::<isize>().ok()).collect_tuple().unwrap();
    Part {x, m, a, s}
}
...
type Data = (HashMap<String, Workflow>, Vec<Part>);
...
mod tests {
    ...
    fn parsing_test() {
        ...
        let g = Guard {op: Op::Greater, value: 2090, key: Key::M};
        assert_eq!(g, parse_guard("m>2090"));
        assert_eq!(Instruction {guard: g, target: Target::Accept}, parse_instruction("m>2090:A"));
        let a = parse_instruction("a<2006:qkq");
        let b = parse_instruction("m>2090:A");
        let default = parse_target("rfg");
        let instructions = vec![a, b];
        assert_eq!((String::from("px"), Workflow {instructions, default}), parse_workflow("px{a<2006:qkq,m>2090:A,rfg}"));
    }
}
